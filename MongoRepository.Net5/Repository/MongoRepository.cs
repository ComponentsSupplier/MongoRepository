namespace MongoRepository.Net5
{
    using MongoDB.Bson;
        ...
        where T : IEntity<TKey>
    {
        ...
        public virtual T GetById(TKey id)
        {
            ...
            var filter = builder.Eq(tobject => tobject.Id, id);
            return this.collection.Find<T>(filter).FirstOrDefault();
        }
        ...
        {
            return this.collection.Find<T>(Builders<T>.Filter.Eq("_id",id)).SingleOrDefault();
        }
        ...
        public virtual void Delete(TKey id)
        {
            if (typeof(T).IsSubclassOf(typeof(Entity)))
            {
                this.collection.DeleteOne(Builders<T>.Filter.Eq("_id", new ObjectId(id as string)));
            }
            else
            {
                this.collection.DeleteOne(Builders<T>.Filter.Eq("_id", BsonValue.Create(id)));
            }
        }
